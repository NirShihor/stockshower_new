...
export interface VolumeBreakoutConfig extends Omit<DatabaseBacktestConfig, 'scoreThreshold'> {
  volumeMultiplier?: number;
  priceBreakoutPercent?: number;
  minBreakoutStrength?: number; // 0-100
}

export class VolumeBreakoutBacktest {
  private config: VolumeBreakoutConfig;
  private dataLoader: HistoricalDataLoader;
  private mt5Simulator: MT5Simulator;
  private breakoutStrategy: VolumeBreakoutStrategy;
  ...
  constructor(config: VolumeBreakoutConfig) {
    ...
    this.breakoutStrategy = new VolumeBreakoutStrategy({
      volumeMultiplier: config.volumeMultiplier || 2.0,
      priceBreakoutPercent: config.priceBreakoutPercent || 0.5,
      lookbackPeriods: 20,
      minATRMove: 0.5,
      requireTrendConfirmation: false // Start without trend filter
    });
  ...
  async run(): Promise<BacktestResults> {
    console.log('Starting volume-based breakout backtest...');
    console.log('Config:', {
      ...this.config,
      volumeMultiplier: this.config.volumeMultiplier || 2.0,
      minBreakoutStrength: this.config.minBreakoutStrength || 60
    });
  ...
  private async processSymbol(symbol: string): Promise<void> {
    try {
      ...
      for (let i = 20; i < candles.length; i++) {
        ...
        if (this.canCreateNewSignal(symbol)) {
          ...
          if (signal) {
            ...
            const breakoutInfo = this.breakoutStrategy.isVolumeBreakout(signal, recentCandles);
            
            if (breakoutInfo.isBreakout && 
                breakoutInfo.strength >= (this.config.minBreakoutStrength || 60)) {
              
              console.log(`Breakout detected: ${symbol} ${breakoutInfo.direction} strength=${breakoutInfo.strength} reason="${breakoutInfo.reason}"`);
              
              // Validate breakout quality
              const validation = this.breakoutStrategy.validateBreakoutQuality(signal, recentCandles);
              
              if (validation.isValid) {
                // Create breakout signal with proper stops/targets
                const breakoutSignal = this.breakoutStrategy.createBreakoutSignal(signal, breakoutInfo);
                ...
                if (filled.length > 0) {
                  ...
                  console.log(`Breakout position opened: ${symbol} ${breakoutInfo.direction} @ ${filled[0].entryPrice}`);
                }
              } else {
                console.log(`Breakout rejected for ${symbol}: ${validation.reason}`);
              }
            }
          }
        }
  ...
  private canCreateNewSignal(symbol: string): boolean {
    ...
    if (lastSignal) {
      ...
      if (timeSinceLastSignal < 30 * 60 * 1000) { // 30 minutes
        return false;
      }
    }
  ...
  private generateResults(): BacktestResults {
    ...
    return {
      ...
      patternPerformance: new Map(), // Not applicable for volume strategy
      dailyStats: []
    };
  }
